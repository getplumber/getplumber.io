...
This creates `.plumber.yaml`: your compliance contract defining 14 controls:
...
12. Pipeline must not override job variables
13. Pipeline must not execute unverified scripts
14. Pipeline must not use Docker-in-Docker
...
If your project uses a custom CI config path, pass it explicitly:

```bash
plumber analyze --gitlab-url https://gitlab.com --project your-group/your-project --ci-config-path .gitlab/ci/main.yml
```
...
include:
  - component: gitlab.com/getplumber/plumber/plumber@v0.1.31
    inputs:
      ci_config_path: $CI_CONFIG_PATH # defaults to project's configured CI config path
