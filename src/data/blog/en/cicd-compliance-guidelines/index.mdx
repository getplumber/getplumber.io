...
- **A 22-point compliance checklist** you can use immediately
...
1. **Pipelines are a prime attack target**
   ...
   Attackers have shifted focus to the software supply chain. Compromising one pipeline can expose code, credentials, and production access across many projects. Incidents like the [tj-actions/changed-files compromise](/blog/tj-actions-compromised), the [Shai-Hulud campaign](/blog/shai-hulud), and the SolarWinds breach demonstrate that workflow security is no longer a niche concern. See [incident details and sources](#supply-chain-incidents-with-sources) below.
...
<div class="border-border/50 bg-background/50 my-6 space-y-5 rounded-xl border p-6">
  ...
  <div>
    ...
    <ol start="14" class="list-decimal space-y-1 pl-6">
...
5. **Start with the [22-point checklist](#cicd-compliance-checklist)** above, then map controls to your regulatory framework using the [mapping table](#control-mapping-iso-27001--nis2--dora--cra).
