...
That `C` headline is the story of the long tail. The popular OSS world keeps the lights on, but on a security-and-supply-chain axis, it has clear, recurring weak spots, and they are the same ones from one project to the next.
...
**5,209 findings** in total. The mass of that number is Medium severity (most of it is the hardcoded-jobs count), which is expected. The interesting pocket is the **698 High or Critical findings**, the kind the industry would actually wake up at 3am for:
...
- **CI debug trace enabled (0 projects)**: nobody in the cohort still ships `CI_DEBUG_TRACE=true` on the default branch. That's a small but real win for the ecosystem compared to previous waves.
