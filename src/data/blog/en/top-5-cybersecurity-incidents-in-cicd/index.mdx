...
authors:
  - r2devops
pubDate: 2024-08-19
heroImage: ../top-5-cybersecurity-incidents-in-cicd/heroImage.png
categories:
  - "News"
  - "Cybersecurity"
  - "CI/CD"
...
CI/CD (Continuous Integration and Continuous Deployment) pipelines changed how we build software: code gets integrated, tested, and deployed in minutes. But as pipelines grew more complex and more automated, they also became a prime target in the software supply chain. Here are five of the highest-profile cybersecurity incidents involving CI/CD pipelines, and what they teach us.
...
The attack exploited weaknesses such as poor control mechanisms and insufficient monitoring within the CI/CD pipeline, which allowed the attackers to maintain a persistent presence and alter the source code without detection.
...
**Issues in the software supply chain:**  
The leak demonstrated poor secret management practices within CI/CD environments, particularly in shared cloud services like Travis CI. Access controls were weak, and sensitive information such as environment variables was not sufficiently protected, creating a significant security risk for projects that depended on Travis CI for their builds.
...
Every one of these incidents went through a CI/CD pipeline, and every breach exploited weaknesses that known security practices would have mitigated.
...
You can check your own pipelines against these risks with the open-source [Plumber CLI](/docs/cli), which audits [GitLab CI/CD](/docs/cli/gitlab) and [GitHub Actions](/docs/cli/github) configurations for the same weaknesses exploited in these incidents. See the full [compliance controls catalog](/docs/cli/controls). For organization-wide, continuous monitoring, there is the [Plumber Platform](/platform).

For more recent incidents, read our analyses of the [Shai-Hulud 2.0 npm attack](/blog/shai-hulud) and the [tj-actions/changed-files compromise](/blog/tj-actions-compromised).
