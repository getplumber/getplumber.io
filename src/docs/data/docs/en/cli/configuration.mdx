...
Four commands serve this mode: [`plumber config generate --overlay`](/docs/cli/reference#plumber-config-generate) writes a minimal overlay starter, [`plumber config resolve`](/docs/cli/reference#plumber-config-resolve) prints the full configuration an overlay expands to, [`plumber config view --explain`](/docs/cli/reference#plumber-config-view) shows per control whether the value comes from the baseline or from your overlay, and [`plumber config slim`](/docs/cli/reference#plumber-config-slim) collapses an existing full config into an overlay.
