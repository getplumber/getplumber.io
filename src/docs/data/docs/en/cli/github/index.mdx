...
This page covers the **GitHub Actions** particulars of the Plumber CLI: how to authenticate, the controls that ship for GitHub, the issues they emit, a complete `github.controls:` configuration block, and the **drop-in [GitHub Action](#github-action)** for automated compliance scanning with SARIF upload. For installation and the provider-agnostic command reference, see the [main CLI page](/docs/cli).
...
<Aside variant="info">
  Requires `git` on `PATH` for local-clone auto-detection. Already present in GitHub Actions runners, the Docker image, and standard macOS / Linux installs.
</Aside>
