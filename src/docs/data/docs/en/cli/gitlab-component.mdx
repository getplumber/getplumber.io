...
3. **Add to your pipeline**
   ...
   include:
     - component: gitlab.com/getplumber/plumber/plumber@v0.1.31
       # inputs:
...
<Tabs defaultValue="import">
  ...
  <TabsContent value="import">
    ...
    3. **Publish a release**
       ...
       - Select an imported tag (e.g., `v0.1.31`) from the branch/tag dropdown
    ...
    5. **Use the component in your pipelines**
       ...
       include:
         - component: gitlab.example.com/infrastructure/plumber/plumber@v0.1.31
           # inputs:
           #   stage: .pre | by default runs in .pre which only runs if there is at least another CI job in another stage
       ```
...
Plumber includes **14 compliance controls**. Each can be enabled/disabled and customized in your `.plumber.yaml`:
...
| **Pipeline must not use Docker-in-Docker** | Detects Docker-in-Docker (dind) services and insecure daemon configurations enabling container escape on shared runners |
...
Detects CI/CD jobs that use Docker-in-Docker (dind) services, which require privileged mode and can enable container escape, lateral movement, and access to secrets from other jobs on shared runners.

When `detectInsecureDaemon` is enabled (default), Plumber also flags jobs where DinD is configured without TLS: `DOCKER_TLS_CERTDIR` set to an empty string or `DOCKER_HOST` using port 2375 (plaintext).
...
Consider using [Kaniko](https://github.com/GoogleContainerTools/kaniko) or [Buildah](https://github.com/containers/buildah) as safer alternatives that do not require privileged mode. See the [CLI documentation](/docs/cli/#available-controls) for full details.
