...
<Tabs defaultValue="homebrew">
  ...
  <TabsContent value="homebrew">
    ...
    **Install a specific version:**
    ...
    ```bash
    brew install getplumber/plumber/plumber@v0.3.51
    ```

    <Aside variant="info">
      Versioned formulas are keg-only. Use the full path (e.g., `/usr/local/opt/plumber@v0.3.51/bin/plumber`) or run `brew link plumber@v0.3.51` to add it to your PATH.
    </Aside>
  </TabsContent>
