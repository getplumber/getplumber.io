...
The provider-agnostic reference for the Plumber CLI. Only the `analyze` command takes provider-specific flags and tokens; the config commands, `plumber explain`, exit codes, and output formats behave identically on GitLab and GitHub. See the [Installation](/docs/cli/installation) page to install the CLI, and the [GitHub](/docs/cli/github) and [GitLab](/docs/cli/gitlab) pages for provider specifics.

## Configuration

Configuration modes are documented on the [Configuration](/docs/cli/configuration) page. The `plumber config` commands are documented below.
...
| `--score` | No | `false` | **Deprecated no-op.** The score banner (letter, points, bar, severity counts) and the score in the JSON, PBOM and CycloneDX output are produced by default; the flag is still accepted so existing invocations do not break |
| `--score-point` | No | `false` | Add the full per-issue-code points breakdown (`Code / Severity / Count / Weight / Cap / Loss`) to stdout and the MR comment, on top of the default banner |
| `--score-push` | No | `false` | Publish this repo's Plumber Score to the hosted badge service. Runs in **CI only** (needs a CI-native OIDC id-token; a local run is a no-op). Every run that has score push enabled publishes its result for its branch. The public badge and the project's score follow the default branch only: a merge-request or pull-request run is stored under its branch and never changes the badge. See [Plumber Score](/docs/plumber-score) |
...
**Contrast:** [`plumber config generate`](#plumber-config-generate) writes the **full** default template **with comments** (or a minimal overlay starter with `--overlay`); `init` writes a **short** file shaped by your answers. Both `init` and the plain `generate` produce a [full configuration](/docs/cli/configuration#full-configuration): nothing is inherited from the baseline afterwards.
...
With `--overlay`, writes a minimal [overlay starter](/docs/cli/configuration#extended-configuration-recommended) instead: a few commented lines beginning with `extends: plumber:default` that inherit the whole baseline. This is the recommended starting point.
...
| `--explain` | `false` | For an [overlay config](/docs/cli/configuration#extended-configuration-recommended): also print, per control, whether the value is inherited from `plumber:default` (`base`) or set in your file (`overlay`) |
...
Resolve `extends` and `includePlumberDefaults` and print the complete effective `.plumber.yaml`. Use it to see exactly what a scan with an [overlay config](/docs/cli/configuration#extended-configuration-recommended) will run, or to materialize an overlay into a full, self-contained file you can commit (nothing left implicit).

With no config file and no `--config`, it prints the built-in default configuration, i.e. what a [zero-config run](/docs/cli/configuration#no-configuration) uses.
...
| `3` | Data collection was degraded (some controls could not be verified): always exits 3, regardless of the score gate |
| `3` | `--fail-warnings` is set and warnings occurred (e.g. an action version that could not be resolved) |
...
Flags that read or publish a verdict have nothing to act on and are ignored, with a one-line notice naming them: `--min-points`, `--min-score`, `--threshold`, `--score`, `--score-point`, `--badge`, `--score-push`, `--mr-comment`, `--platform`, plus the two security reports above. No badge is created or updated, and no score is published.
...
| `plumberScore` | object | Scored summary: `profileId` (`scoring-v3`), per-severity `counts`, `rawPoints`, `finalPoints`, `score` (A-E), `criticalMalusApplied`, `losses` and `codeLosses`. Written on every run; omitted when the score is withheld because no control was evaluated. |
...
| `dataCollectionDegraded` | boolean | True when a collection or enrichment step failed mid-run, so the analysis ran on incomplete data. A degraded run always exits with code 3, regardless of the score gate, so it never exits 0. Omitted when false. |
| `degradedReasons` | array | Human-readable reasons behind `dataCollectionDegraded`. Omitted when not degraded. |
| `plumberConfig` | object | Self-describing snapshot of the effective policy: `source`, `effectivePolicy` (the parsed config with comments stripped and, for an [overlay config](/docs/cli/configuration#extended-configuration-recommended), `extends` fully resolved), and `hash` (sha256 of the canonical policy). Written on every run. |
