...
and/or `Redis`. The chart can optionally deploy a standalone Redis instance
for you. Both alternatives are detailed below.
...
4. ### ⚙️ Configure your values
   ...
   **Option A: Deploy Redis via the chart**
   ...
   redis:
     ...
     # Persistence is disabled by default (Redis is used as a cache/queue).
     # Uncomment to enable:
     #master:
     #  persistence:
     #    enabled: false

     # To pull from a private registry, uncomment and configure:
     #image:
...
5. ### 🚀 Install the chart
   ...
   If you enabled `redis.deploy: true`, update chart dependencies first:
   ```sh
   helm dependency update plumber/plumber
   ```
