...
<Aside variant="info">
Credentials are **not** configured here. They are stored encrypted in the Platform database: the
GitLab OAuth application is set once by `plumber-bootstrap`; the GitLab access token, SMTP and the
licence are set in `Settings` as an Admin.
</Aside>
