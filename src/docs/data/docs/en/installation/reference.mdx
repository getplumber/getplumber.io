...
On Docker Compose installs these values are set inside `compose.yml` itself; changing them in `.env` has no effect. Edit the compose file if you must override them.
</Aside>
