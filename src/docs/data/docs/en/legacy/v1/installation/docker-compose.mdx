...
Choose option **1 (Production)**: type `1` and press Enter. The installer will guide you through:
...
perl -pi -e "s/^JOBS_DB_PASSWORD=.*/JOBS_DB_PASSWORD=\"$(openssl rand -hex 16)\"/" .env
perl -pi -e "s/^JOBS_REDIS_PASSWORD=.*/JOBS_REDIS_PASSWORD=\"$(openssl rand -hex 16)\"/" .env
```
