...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@0710cef67930a54cc9d9630d0b7166bf702bd118 # pinned plumber version: v0.4.59
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@c4e322657aba1c12282e7eac5bfd9481c5bf700a # pinned plumber version: v0.4.59
        inputs:
          score_push: true
    ```
