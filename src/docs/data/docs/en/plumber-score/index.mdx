...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@00650e5b55507c80a69ddd1507632a139b6087e5 # pinned plumber version: v0.4.37
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@71f85f223bfa4d17fc53858c467abd7455d78dff # pinned plumber version: v0.4.37
        inputs:
          score_push: true
    ```
