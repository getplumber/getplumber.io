...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@12040938aaf7ec1a0afa37c5f6cab50d7a61f6c3 # pinned plumber version: v0.4.27
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@b76c7079acfee5e3a2a73b7b8d6d70fd4eb2256b # pinned plumber version: v0.4.27
        inputs:
          score_push: true
    ```
