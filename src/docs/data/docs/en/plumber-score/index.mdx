...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@94bcea5b75e83af4606c120b8e701314a5c10dcd # pinned plumber version: v0.4.32
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@cbaa720174cbf1ee0c7f5902cefb0f48e03489a2 # pinned plumber version: v0.4.32
        inputs:
          score_push: true
    ```
