...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@8a58d655cbad3865a09c5edef51832cd695aaeb1 # pinned plumber version: v0.4.33
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@0f42336d16e0b0b50068b6efb45c6f61eb53430e # pinned plumber version: v0.4.33
        inputs:
          score_push: true
    ```
