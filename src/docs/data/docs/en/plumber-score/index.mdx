...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@709877722bc85421bd0c2188f818beb4038842a3 # pinned plumber version: v0.4.43
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@3ebe64b56d5dfdbe013c957806d286bbf9d7f61c # pinned plumber version: v0.4.43
        inputs:
          score_push: true
    ```
