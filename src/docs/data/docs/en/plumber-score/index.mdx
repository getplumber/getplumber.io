...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@ce2d9603cc9faf10df2b76b276366104a61c68d0 # pinned plumber version: v0.4.31
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@ab242332a8983aa2edf15700b253442a15b640b0 # pinned plumber version: v0.4.31
        inputs:
          score_push: true
    ```
