...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@b6fe9f309d4242b82ce83e6e69c563c09792ca97 # pinned plumber version: v0.4.22
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@d8a8b7e18c1df5009abbd18f1f432b10efc9f44e # pinned plumber version: v0.4.22
        inputs:
          score_push: true
    ```
