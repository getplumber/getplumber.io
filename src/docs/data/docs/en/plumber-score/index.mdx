...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@d37150cee04d6627497cb4c6a553ef9a2d6c7548 # pinned plumber version: v0.5.2
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@a4cae41a56ba74bfd31e5f84e9e5ce26637c420c # pinned plumber version: v0.5.2
        inputs:
          score_push: true
    ```
