...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@382898852b285f7c1a6dff2f0ff03258408741da # pinned plumber version: v0.5.1
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@ac301139f1ee7b0fe13204a542ab506f237e4746 # pinned plumber version: v0.5.1
        inputs:
          score_push: true
    ```
