...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@24de99968c08ffe230545f413695ad933208b0d2 # pinned plumber version: v0.4.29
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@dde5d234a611f44bdd7c58d610c2d1d329b4da51 # pinned plumber version: v0.4.29
        inputs:
          score_push: true
    ```
