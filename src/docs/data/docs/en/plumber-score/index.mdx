...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@c85c712bca3c9f547a88c66bb2541db179a1a79e # pinned plumber version: v0.4.52
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@b034cbacd72b35c5c3fdce4385bdbbe3c83a6d54 # pinned plumber version: v0.4.52
        inputs:
          score_push: true
    ```
