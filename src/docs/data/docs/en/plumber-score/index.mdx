...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@56369e7d43c4878049ee8ec6c2d600c21af45c4e # pinned plumber version: v0.4.40
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@b735294fceab2f0e460a0cb7f4a14ea4b1cd4aca # pinned plumber version: v0.4.40
        inputs:
          score_push: true
    ```
