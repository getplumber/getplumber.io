...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@4106f1120ba8db839bce0f213cf7b3ad3927cc5f # pinned plumber version: v0.4.50
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@c6254249236b01c879d8c43cf5525b659db2ba70 # pinned plumber version: v0.4.50
        inputs:
          score_push: true
    ```
