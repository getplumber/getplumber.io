...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@1f0d4be4239f44f26a851c758d2cb7112919dd0c # pinned plumber version: v0.4.57
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@740eead884d0368ada71705d64db9a957567b5a9 # pinned plumber version: v0.4.57
        inputs:
          score_push: true
    ```
