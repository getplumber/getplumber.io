...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@031bc86d1e5f661626e2000f4f8779912d608d5f # pinned plumber version: v0.4.39
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@bc5f7162f21863f56366cfd6b1ef347a59a84941 # pinned plumber version: v0.4.39
        inputs:
          score_push: true
    ```
