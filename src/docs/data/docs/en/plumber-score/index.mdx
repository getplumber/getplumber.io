...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@d3a48e9d701a2ca1f7aa108b7d854e01c0f125c6 # pinned plumber version: v0.4.24
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@3c5638133b513b7276bbbf24fc5e6d5b66e3bfbe # pinned plumber version: v0.4.24
        inputs:
          score_push: true
    ```
