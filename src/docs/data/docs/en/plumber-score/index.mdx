...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@3feac69e925e9771f8a495f4177af754d568c1ad # pinned plumber version: v0.4.48
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@e3aedaceae34cce86cd03f40b4531a61b9a0c674 # pinned plumber version: v0.4.48
        inputs:
          score_push: true
    ```
