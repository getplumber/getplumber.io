...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@87ca135cc4566ba8e9065ea95b5d897232cdb208 # pinned plumber version: v0.4.16
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@4f88b0cbf9169c57ce174fecdb5484e2f73cb0cd # pinned plumber version: v0.4.16
        inputs:
          score_push: true
    ```
