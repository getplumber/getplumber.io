...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@6452d3d9c12dfa4aecaecde6bc6c528eea3b21a5 # pinned plumber version: v0.4.55
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@d949b4fb2f2fa7355fbe2ff9d13d74e8512dd3d5 # pinned plumber version: v0.4.55
        inputs:
          score_push: true
    ```
