...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@238d6405de33b049954607f88ffeab2c8e43c13a # pinned plumber version: v0.4.25
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@2d5b848fc327c6811501b0b5b2903142eeac7b1a # pinned plumber version: v0.4.25
        inputs:
          score_push: true
    ```
