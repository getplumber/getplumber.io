...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@ada53a23e2be3590d075360d7c2a155801e25fce # pinned plumber version: v0.4.38
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@0da6bde4f333ce7580719aa08e92f589e6f24c53 # pinned plumber version: v0.4.38
        inputs:
          score_push: true
    ```
