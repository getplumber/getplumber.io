...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@f84005b69dc57965d192feedd08b4a20fa6e52cc # pinned plumber version: v0.4.60
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@4876a79195f5ae6bcc8a04c7c89363a801bd44ad # pinned plumber version: v0.4.60
        inputs:
          score_push: true
    ```
