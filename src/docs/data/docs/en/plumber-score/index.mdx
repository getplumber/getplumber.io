...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@cc5816b9724c1b4a264708ab4614b077313d3e16 # pinned plumber version: v0.4.18
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@3c3754b6e93f343d9739375256475b38e7fb2a7c # pinned plumber version: v0.4.18
        inputs:
          score_push: true
    ```
