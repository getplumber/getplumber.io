...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@bede38d25e270af86c2cec5dadc75e0925eb319c # pinned plumber version: v0.4.44
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@075f1889dc45ddc0630159f95fe4c447491d22a5 # pinned plumber version: v0.4.44
        inputs:
          score_push: true
    ```
