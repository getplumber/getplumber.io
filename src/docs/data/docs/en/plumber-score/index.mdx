...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@db6f8ea7486ecffaf6aac462b285979fcc53198c # pinned plumber version: v0.4.58
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@36363aad976e0b722358d5a89bb2553632d5e504 # pinned plumber version: v0.4.58
        inputs:
          score_push: true
    ```
