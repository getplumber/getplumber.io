...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@932f3b5b87ffdd6f3101a2e6ce5a223320960849 # pinned plumber version: v0.4.35
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@43e8538c023608e4caeac9d6e7de0518846821a5 # pinned plumber version: v0.4.35
        inputs:
          score_push: true
    ```
