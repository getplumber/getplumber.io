...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@95c19b0d5cf54d1aeecbd78ceda33cc32581b093 # pinned plumber version: v0.4.30
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@4102242597c475ed83cb6f2cf302210f0ce61367 # pinned plumber version: v0.4.30
        inputs:
          score_push: true
    ```
