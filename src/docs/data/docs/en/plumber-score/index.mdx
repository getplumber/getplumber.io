...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@d65469981e29ba7069d9fcef334e2fc41490c420 # pinned plumber version: v0.4.47
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@898a9d1590f72682ad4a0865a14a6a17f2f3fc9f # pinned plumber version: v0.4.47
        inputs:
          score_push: true
    ```
