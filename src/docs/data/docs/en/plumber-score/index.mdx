...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@e815e0bff47f9170e8e0a056df0a851124634ae2 # pinned plumber version: v0.4.51
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@de6c51afb28f0becedc1dcde385321bcb95d8512 # pinned plumber version: v0.4.51
        inputs:
          score_push: true
    ```
