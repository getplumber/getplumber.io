...
with the correct workflow `permissions` and `score-push` enabled in the action
    ...
    jobs:
      plumber:
        steps:
          - uses: getplumber/plumber@f4080f98de83e6b2a2be3bf41e971080ec982357 # pinned plumber version: v0.4.21
            with:
              score-push: true
    ```
...
`.gitlab-ci.yml` with the `score_push` input enabled
    ```yaml
    include:
      - component: gitlab.com/getplumber/plumber/plumber@c1ecf1a54146b81eb8555648955270048982523b # pinned plumber version: v0.4.21
        inputs:
          score_push: true
    ```
